...
# 用户 包括教科办管理员 教师 ROOT 这里不用于
DROP TABLE IF EXISTS `user`;
CREATE TABLE `user` (
  `user_id` INT(8) NOT NULL, # teacher id
  `user_name` VARCHAR(24) NOT NULL, # Hadi Barzegar Bafrooei 老师的真名
	`user_role` TINYINT(1) UNSIGNED DEFAULT NULL, # 0 1 2 teacher manager root
	`user_avatar` VARCHAR(24) DEFAULT NULL, # 头像相对地址！ 前端上传成功以后 再调用我们接口存储这个地址 设置相对地址 防止域名变化导致问题！
  PRIMARY KEY (`user_id`),
