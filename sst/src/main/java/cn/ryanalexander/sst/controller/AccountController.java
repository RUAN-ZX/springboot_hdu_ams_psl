...
public class AccountController {
    ...
    public Object getCaptcha(String accountId) throws Exception {
        ...
        System.out.println(accountFeignService.getCaptcha(accountId,"").getData());
        return Temail;
    }
    @ApiOperation("未注册 未登记的邮箱 获取验证码")
    @GetMapping("/sendCaptcha")
    public Object sendCaptcha(String mailTo){
        System.out.println(accountFeignService.sendCaptcha(mailTo).getData());
        return mailTo;
    }
...
//        }// 普通密码
        ...
        if(AccountService.isCaptcha(accountPwd)){
            accountFeignService.verifyCaptcha(accountId,accountPwd);
        }
